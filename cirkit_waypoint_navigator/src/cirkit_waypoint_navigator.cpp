...
public:
  WayPoint();
  WayPoint(move_base_msgs::MoveBaseGoal goal, int area_type,
...
public:
  ...
  bool isAlreadyApproachedToTargetObject(
      jsk_recognition_msgs::BoundingBox target_object);

  double ComputeDistanceToWaypoint(geometry_msgs::Pose a,
                                   geometry_msgs::Pose b);

  void SendNextGoal(jsk_recognition_msgs::BoundingBox target_object,
                    double threshold);
  ...
  geometry_msgs::Pose
  getTargetObjectApproachPosition(geometry_msgs::Pose target_position,
                                  double tolerance);
...
private:
  ...
  jsk_recognition_msgs::BoundingBoxArray
      approached_target_objects_; //アプローチ済みの探索対象
  double
      dist_thres_to_target_object_; // 探索対象にどれだけ近づいたらゴールとするか
  double
      reach_threshold_; // 今セットされてるゴール（waypointもしくは探索対象）へのしきい値
...
CirkitWaypointNavigator::CirkitWaypointNavigator()
  ...
  detect_target_objects_sub_ =
      nh_.subscribe("/recognized_result", 1,
                    &CirkitWaypointNavigator::detectTargetObjectCallback, this);
  next_waypoint_marker_pub_ =
      nh_.advertise<visualization_msgs::Marker>("/next_waypoint", 1);

  // Register Service Client.
  detect_target_object_monitor_client_ =
      nh_.serviceClient<cirkit_waypoint_navigator::TeleportAbsolute>(
...
int CirkitWaypointNavigator::readWaypoint(std::string filename) {
  ...
  while (ifs.good()) {
    ...
    if (data.size() != rows_num) {
      ...
      waypoints_buff_.push_back(
          WayPoint(waypoint, (int)data[7], data[8] / 2.0));
    }
  }
  return 0;
}

void CirkitWaypointNavigator::detectTargetObjectCallback(
    const jsk_recognition_msgs::BoundingBoxArray::ConstPtr
        &target_objects_ptr) {
...
bool CirkitWaypointNavigator::isAlreadyApproachedToTargetObject(
    jsk_recognition_msgs::BoundingBox target_object) {
  for (int i = 0; i < approached_target_objects_.boxes.size(); ++i) {
    double dist = ComputeDistanceToWaypoint(
        target_object.pose, approached_target_objects_.boxes[i].pose);
...
// 探索対象へのアプローチの場合
void CirkitWaypointNavigator::SendNextGoal(
    jsk_recognition_msgs::BoundingBox target_object, double threshold) {
  reach_threshold_ = threshold;
  // 現在のロボットの位置と探索対象を中心とした円の交点座標のロボットに近い方
  geometry_msgs::Pose approach_pos =
...
double CirkitWaypointNavigator::getReachThreshold() { return reach_threshold_; }
...
geometry_msgs::Pose CirkitWaypointNavigator::getNowGoalPosition() {
  // ROS_INFO_STREAM("g)x :" << now_goal_.position.x << ", y :" <<
  // now_goal_.position.y);
  return now_goal_;
}

void CirkitWaypointNavigator::tryBackRecovery() {
  ROS_INFO_STREAM("Start tryBackRecovery()");
  laser_scan_sub_ = nh_.subscribe(
      "scan_multi", 1, &CirkitWaypointNavigator::laserCallback, this);
  cmd_vel_pub_ = nh_.advertise<geometry_msgs::Twist>("/cmd_vel", 1);
  geometry_msgs::Twist msg;
  geometry_msgs::Pose start_recovery_position =
      this->GetRobotCurrentPosition(); // 現在座標
  ...
  while (ros::ok()) {
    ...
    geometry_msgs::Pose robot_current_position =
        this->GetRobotCurrentPosition(); // 現在のロボットの座標
    double delta_dist = this->ComputeDistanceToWaypoint(
...
void CirkitWaypointNavigator::laserCallback(
    const sensor_msgs::LaserScan::ConstPtr &scan) {
  projector_.projectLaser(*scan, cloud_);
}

void CirkitWaypointNavigator::sendApproachedTargetPosition() {
  jsk_recognition_msgs::BoundingBox approached_target_object =
      approached_target_objects_.boxes.back();
...
geometry_msgs::Pose CirkitWaypointNavigator::getTargetObjectApproachPosition(
    geometry_msgs::Pose target_position, double tolerance) {
  ...
  if ((robot_position.position.x - target_position.position.x) == 0.0) {
    ...
    answers[0].position.x =
        target_position.position.x + tolerance / sqrt(1 + pow(C, 2.0));
    answers[0].position.y =
        target_position.position.y + (C * tolerance) / sqrt(1 + pow(C, 2.0));
    answers[1].position.x =
        target_position.position.x - tolerance / sqrt(1 + pow(C, 2.0));
    answers[1].position.y =
        target_position.position.y - (C * tolerance) / sqrt(1 + pow(C, 2.0));
  }
  for (size_t i = 0; i < 2; ++i) {
    answers[i].orientation = target_position.orientation;
    distances[i] = this->ComputeDistanceToWaypoint(robot_position, answers[i]);
  }
  ...
  while (ros::ok()) {
    ...
    if (next_waypoint.isSearchArea()) {
      ...
      if (target_objects_.boxes.size() > 0) {
        ROS_INFO_STREAM(
            "Found target objects : " << target_objects_.boxes.size());
        for (int i = 0; i < target_objects_.boxes.size(); ++i) {
          if (!this->isAlreadyApproachedToTargetObject(
                  target_objects_
                      .boxes[i])) { // 探索対象にまだアプローチしていなかったら
            //今の位置から5[m]以内なら目指す
            ...
            double distance_to_target =
                this->ComputeDistanceToWaypoint(robot_pose, target_pose);
            if (distance_to_target < 5.0) {
              ROS_INFO_STREAM("Found new target object.");
              this->SendNextGoal(
                  target_objects_.boxes[i],
                  dist_thres_to_target_object_); // 探索対象を次のゴールに設定
        ...
        if (!is_set_next_as_target) {
          ...
          ROS_INFO_STREAM("Valid target object isn't founded, hence next "
                          "waypoint is set.");
        }
    ...
    ros::Time begin_navigation =
        ros::Time::now(); // 新しいナビゲーションを設定した時間
    ...
    while (ros::ok()) {
      ...
      geometry_msgs::Pose now_goal_position = this->getNowGoalPosition();

      // X. Compute distance to reach targeted waypoint.
      double distance_to_goal = this->ComputeDistanceToWaypoint(
          robot_current_position, now_goal_position);
      ...
      if (delta_distance_to_goal < 0.1) {
        ...
        if (how_long_stay_time.toSec() > 10.0) {
          if (robot_behavior_state_ == RobotBehaviors::WAYPOINT_NAV) {
            ...
            break;
          } else if (robot_behavior_state_ ==
                     RobotBehaviors::DETECT_TARGET_NAV) {
            robot_behavior_state_ =
                RobotBehaviors::DETECT_TARGET_NAV_PLANNING_ABORTED;
            break;
          } else {
            break;
          }
        ...
        // If robot moves more than 0.1m, OK.
    ...
    case RobotBehaviors::DETECT_TARGET_NAV_PLANNING_ABORTED: {
      ...
      if (number_of_approached_to_target_ > limit_of_approach_to_target_) {
        ...
        ROS_RED_STREAM("Faild to approach ... "
                       << number_of_approached_to_target_ << "times");
