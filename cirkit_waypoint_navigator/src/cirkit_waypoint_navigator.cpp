...
void CirkitWaypointNavigator::HandleLocalCostMap(
  ...
  ROS_ERROR("Receive OGM");
  ROS_ERROR("Width : %u, Height : %u", grid_map->info.width,
            grid_map->info.height);
  ROS_ERROR("Origin.x : %lf, Origin.y : %lf", grid_map->info.origin.position.x,
            grid_map->info.origin.position.y);
  ROS_ERROR("Resolution : %lf", grid_map->info.resolution);
  ROS_ERROR("Frame ID : %s", grid_map->header.frame_id.c_str());
  double yaw = tf::getYaw(grid_map->info.origin.orientation);
  ROS_ERROR("Yaw : %lf", yaw);
  ROS_ERROR("Size : %d", grid_map->data.size());

  cv::Mat mat = cv::Mat::zeros(120, 120, CV_8SC1);

  for (int j = 0; j < 120; j++) {
    for (int i = 0; i < 120; i++) {
      mat.at<int8_t>(j, i) = grid_map->data[120 * i + j];
    }
  }
